...
export default function SearchDialog() {
    ...
    return (
        <Fragment>
            ...
            {/* Dialog  */}
            <Dialog className=" relative right-[1em] w-[25em]  md:right-0 md:w-0 lg:right-0 lg:w-0" open={open} handler={handleOpen} style={{ background: mode === 'light' ? 'rgb(111 102 195)' : 'rgb(24 29 40)', color: mode === 'dark' ? 'white' : 'black' }}>
                {/* Dialog Body  */}
                <DialogBody >
                                ...
                                return (
                                    <div key={index} className="p-2 sm:w-1/4 w-full " >
                                        <div onClick={() => naviagte(`/bloginfo/${item.id}`)} className=" container cursor-pointer mx-auto px-4 bg-gray-200 p-2 rounded-lg ">
                                            ...
                                            {/* Blog Date  */}
                                            <p className="w-40 text-[12px]">{item.date}</p>

                                            {/* Blog Title  */}
                                            <h1 className="font-bold text-gray-700">{item.blogs.title}</h1>
                                        </div>
                                    </div>
                                )
                            })}
                    ...
                    <div className=" text-center">
                        <h1 style={{ color : mode === "dark" ? "rgb(117 117 117)" : "#978df3"}}>Powered By UXKD</h1>
                    </div>
                </DialogBody>
            </Dialog>
        </Fragment>
